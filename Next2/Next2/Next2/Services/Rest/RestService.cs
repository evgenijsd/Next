...
namespace Next2.Services.Rest
{
    public class RestService : IRestService
    {
        ...
        public RestService(ISettingsManager settingsManager, IAmazonService amazonService)
        {
        ...
        private async Task<HttpResponseMessage> MakeRequestAsync(string requestUrl, HttpMethod method, object requestBody = null, Dictionary<string, string> additioalHeaders = null)
        {
            ...
            if (_settingsManager.AuthComplete)
            {
                ...
                await _amazonService.SignHeadersAsync(request);
            }
        ...
        {
            if (DateTime.TryParse(_settingsManager.AccessSettings.TokenExpirationDate, CultureInfo.InvariantCulture, DateTimeStyles.None, out DateTime dt) && dt < DateTime.Now)
            {
        ...
        {
            var res = await _amazonService.RefreshTokenAsync();
            ...
            {
                await _amazonService.LogoutAsync();
